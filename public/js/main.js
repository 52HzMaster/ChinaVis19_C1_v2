...
function initCamera() {
    ...
    //camera = new THREE.OrthographicCamera(-30, 30, 16, -16,geo_height, 100);
...
function initModel() {
    ...
    draw_f1_area(0.1,"#4f51ff",0.3);
    draw_f2_area(0.1,"#4f51ff",0.3);
...
function draw_f1_area(geo_height,color,opacity){
    ...
    //f1_areaMain.material.depthTest = false;
    ...
    //f1_areaA.material.depthTest = false;
    ...
    //f1_areaB.material.depthTest = false;
    ...
    //f1_areaC.material.depthTest = false;
    ...
    //f1_areaD.material.depthTest = false;
    ...
    //f1_areaDisc.material.depthTest = false;
    ...
    //f1_areaServe.material.depthTest = false;
    ...
    //f1_areaPoster.material.depthTest = false;
    ...
    //f1_areaSign.material.depthTest = false;
    ...
    //f1_room1.material.depthTest = false;
    ...
    //f1_room2.material.depthTest = false;
    ...
    //f1_room3.material.depthTest = false;
    ...
    //f1_room4.material.depthTest = false;
    ...
    //f1_wc1.material.depthTest = false;
    f1_wc1.position.x = -4;
    ...
    //f1_wc2.material.depthTest = false;
...
function draw_f2_area(geo_height,color,opacity){

    let f2_group = new THREE.Group();
    f2_group.name = 'f2_group';
    scene.add(f2_group);
    ...
    /*let f2_wc3 = new THREE.Mesh(new THREE.BoxGeometry(2,geo_height, 2));
     f2_wc3.name = "area_wc3";
     f2_wc3.material.color.set(color);
     f2_wc3.material.transparent = true;
     f2_wc3.material.needsUpdate = true;
     f2_wc3.material.opacity = opacity;
     //f2_wc3.material.depthTest = false;
     f2_wc3.position.x = -4;
     f2_wc3.position.y = geo_height/2+15.01;
     f2_wc3.position.z = -33;
     scene.add(f2_wc3);

     let f2_canteen = new THREE.Mesh(new THREE.BoxGeometry(5,geo_height, 8));
     f2_canteen.name = "area_canteen";
     f2_canteen.material.color.set(color);
     f2_canteen.material.transparent = true;
     f2_canteen.material.needsUpdate = true;
     f2_canteen.material.opacity = opacity;
     //f2_canteen.material.depthTest = false;
     f2_canteen.position.x = -11.5;
     f2_canteen.position.y = geo_height/2+15.01;
     f2_canteen.position.z = -32;
     scene.add(f2_canteen);

     let f2_leisure = new THREE.Mesh(new THREE.BoxGeometry(6,geo_height, 3));
     f2_leisure.name = "area_leisure";
     f2_leisure.material.color.set(color);
     f2_leisure.material.transparent = true;
     f2_leisure.material.needsUpdate = true;
     f2_leisure.material.opacity = opacity;
     //f2_leisure.material.depthTest = false;
     f2_leisure.position.x = -12;
     f2_leisure.position.y = geo_height/2+15.01;
     f2_leisure.position.z = -23.5;
     scene.add(f2_leisure);

     let f2_room5 = new THREE.Mesh(new THREE.BoxGeometry(5,geo_height, 2));
     f2_room5.name = "area_room5";
     f2_room5.material.color.set(color);
     f2_room5.material.transparent = true;
     f2_room5.material.needsUpdate = true;
     f2_room5.material.opacity = opacity;
     //f2_room5.material.depthTest = false;
     f2_room5.position.x = -11.5;
     f2_room5.position.y = geo_height/2+15.01;
     f2_room5.position.z = -27;
     scene.add(f2_room5);

     let f2_room6 = new THREE.Mesh(new THREE.BoxGeometry(2,geo_height, 2));
     f2_room6.name = "area_room6";
     f2_room6.material.color.set(color);
     f2_room6.material.transparent = true;
     f2_room6.material.needsUpdate = true;
     f2_room6.material.opacity = opacity;
     //f2_room6.material.depthTest = false;
     f2_room6.position.x = -4;
     f2_room6.position.y = geo_height/2+15.01;
     f2_room6.position.z = -31;
     scene.add(f2_room6);*/
...
function draw_group_traj(index,group,param) {

    if(scene.getObjectByName('traj_group'))
        scene.remove(scene.getObjectByName("traj_group"));
        ...
        success: function (data, textStatus) {
            ...
            data.forEach((d,i)=>{
                ...
                d.traj.forEach((s)=>{
                    ...
                    if( s.floor ===1){
                        if(points_3d.length>0){
                            console.log(Math.abs(s.coor[1]-14.5 - points_3d[points_3d.length-1].x)>1);
                            console.log(Math.abs(s.coor[0]-7.5 - points_3d[points_3d.length-1].z)>1);
                        }
                        points_3d.push(new THREE.Vector3( s.coor[1]-14.5, stay_scale(s.stay), s.coor[0]-7.5 ));
                    }
                    else
                        points_3d_f2.push(new THREE.Vector3( s.coor[1]-14.5 , stay_scale(s.stay), s.coor[0]-7.5-16  ));
                });
                let geometry = new THREE.Geometry();
                geometry.vertices = new THREE.CatmullRomCurve3(points_3d).getPoints( 1000 );
                let line = new THREE.Line(geometry, new THREE.LineBasicMaterial({color:param,transparent: true, opacity: 0.2}));
                line.name = "traj_"+d.id;
                type_group.add(line);
                if(points_3d_f2.length>1){
                    let geometry = new THREE.Geometry();
                    geometry.vertices = new THREE.CatmullRomCurve3(points_3d_f2).getPoints( 1000 );
                    type_group.add(new THREE.Line(geometry, new THREE.LineBasicMaterial({color:param,transparent: true, opacity: 0.2})));
                }
            });
...
function draw_one_traj(id) {

    if(scene.getObjectByName("one_traj")){
        scene.remove(scene.getObjectByName('one_traj'))
    }
    $.ajax({
        url: day_url+"_one_traj",    //请求的url地址
        dataType: "json",   //返回格式为json
        data:{id:id},
        async: true, //请求是否异步，默认为异步，这也是ajax重要特性
        type: "GET",   //请求方式
        ...
        success: function (data, textStatus) {
            ...
            let geometry = new THREE.Geometry();
            geometry.vertices = new THREE.CatmullRomCurve3(points_3d).getPoints( 1000 );
            let line = new THREE.Line(geometry, new THREE.LineBasicMaterial({color:"#ff333d",transparent: true, opacity: 0.2}));
            line.name = "one_traj";
            scene.add(line);
            // if(points_3d_f2.length>1){
            //     let geometry = new THREE.Geometry();
            //     geometry.vertices = new THREE.CatmullRomCurve3(points_3d_f2).getPoints( 1000 );
            //     scene.add(new THREE.Line(geometry, new THREE.LineBasicMaterial({color:param,transparent: true, opacity: 0.2})));
            // }
        },
        complete: function () {//请求完成的处理
        },
...
function heatmap_3d(date,floor) {
    ...
    scene.getObjectByName("f1_group").visible = false;
    d3.select("#main_lg").style("display","block");
        ...
        success: function (dataset, textStatus) {
            ...
            dataset.forEach((d)=>{
                ...
                if(d.data&&floor ===1){
                    ...
                    //sensor.material.depthTest = false;
...
function onMouseClick( event ) {
    ...
    let lines = raycaster.intersectObjects( scene.getObjectByName('traj_group').children);
    ...
    if(lines.length>0) {
        ...
        draw_one_traj(lines[0].object.name.replace("traj_",""));
        scene.getObjectByName('traj_group').visible = false;
        lines = null;
    }
    else{
        scene.getObjectByName('traj_group').children.forEach(function(d){
            d.visible = true;
        });
    }
...
function onHover( event ) {
    ...
    if(lines.length>0) {
        //console.log(intersects);
        document.body.style.cursor = 'pointer';
        scene.getObjectByName(lines[0].object.name).material.color.set("#36ff31");
        d3.select(".id_info").text("人员ID: "+lines[0].object.name.replace("traj_",""));
        lines = null;
    }
    else{
        scene.getObjectByName('traj_group').children.forEach(function(d){
            d.material.color.set("#ff333d");
        });
    ...
    .on("click",function (d) {
        if(d === 'refresh'){
            if(scene.getObjectByName('sensor_group1')){
                ...
                scene.remove(scene.getObjectByName('sensor_group2'));
            }
            scene.remove(scene.getObjectByName("traj_group"));
            ...
            d3.select("#main_lg").style("display","none")
            d3.select("#main_date").style("display","none")
        }
    })
